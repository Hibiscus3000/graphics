...
public abstract class MultiplePointsInstrument extends RotatableInstrument {
    ...
    protected MultiplePointsInstrument(ParametersParser parametersParser, InstrumentUser instrumentUser,
                                       int needPointsMax) {
    ...
    @Override
    protected synchronized void changeSize(int mouseWheelClicks) {
        for (String paramName : getResizableParams()) {
            changeValue(paramName, mouseWheelClicks);
        }
    }
